...
-- ── Triggers (drop first so they're idempotent) ───────────────
DROP TRIGGER IF EXISTS members_updated_at            ON members;
DROP TRIGGER IF EXISTS tracks_updated_at             ON tracks;
DROP TRIGGER IF EXISTS modules_updated_at            ON modules;
DROP TRIGGER IF EXISTS discipleship_groups_updated_at ON discipleship_groups;
DROP TRIGGER IF EXISTS member_progress_updated_at    ON member_progress;
DROP TRIGGER IF EXISTS member_progress_completed_at  ON member_progress;
...
CREATE TRIGGER discipleship_groups_updated_at
  BEFORE UPDATE ON discipleship_groups FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER member_progress_updated_at
  BEFORE UPDATE ON member_progress FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER member_progress_completed_at
  BEFORE UPDATE ON member_progress FOR EACH ROW EXECUTE FUNCTION set_completed_at();
...
LEFT JOIN member_progress mp ON mp.member_id = m.id AND mp.module_id = mo.id
