-- ============================================================
-- THE DISCIPLES — Row Level Security Policies
...
-- Returns true when the caller leads a group that contains member_id
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    ...
    WHERE g.discipler_id = get_my_member_id()
      AND gm.member_id   = p_member_id
  )
$$ LANGUAGE sql SECURITY DEFINER STABLE;

-- Returns true when caller is in a group led by the meeting's group discipler
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    ...
    WHERE mt.id          = p_meeting_id
      AND gm.member_id   = get_my_member_id()
  )
$$ LANGUAGE sql SECURITY DEFINER STABLE;
...
-- ── ROLES table ───────────────────────────────────────────────
-- Everyone can read roles; only admins write
...
-- Discipler: read own profile + members they lead
CREATE POLICY "members_discipler_select"
  ON members FOR SELECT
  TO authenticated
  USING (
    is_discipler() AND (
      id = get_my_member_id()
      OR i_lead_this_member(id)
    )
  );

-- Disciple: read + update own profile only
...
-- ── TRACKS table ─────────────────────────────────────────────
-- Everyone reads active tracks; admins write
...
-- ── DISCIPLESHIP_GROUPS table ─────────────────────────────────
CREATE POLICY "groups_admin_all"
  ON discipleship_groups FOR ALL
  TO authenticated
  USING (is_admin())
  WITH CHECK (is_admin());

-- Discipler sees own groups
CREATE POLICY "groups_discipler_select"
  ON discipleship_groups FOR SELECT
  TO authenticated
  USING (discipler_id = get_my_member_id());

-- Disciple sees groups they belong to
...
-- Discipler manages members of own groups
CREATE POLICY "group_members_discipler_all"
  ON group_members FOR ALL
  TO authenticated
  USING (
    EXISTS (
      ...
      WHERE g.id           = group_id
        AND g.discipler_id = get_my_member_id()
    )
  )
  WITH CHECK (
    EXISTS (
...
-- Discipler reads/writes progress for their group members
CREATE POLICY "progress_discipler_all"
  ON member_progress FOR ALL
  TO authenticated
  USING (is_discipler() AND i_lead_this_member(member_id))
  WITH CHECK (is_discipler() AND i_lead_this_member(member_id));

-- Disciple reads/updates own progress
...
-- Discipler CRUD meetings for own groups
CREATE POLICY "meetings_discipler_all"
  ON meetings FOR ALL
  TO authenticated
  USING (
    EXISTS (
...
-- Disciple reads meetings for own group
...
-- Discipler CRUD attendance for own group meetings
CREATE POLICY "attendance_discipler_all"
  ON attendance FOR ALL
  TO authenticated
  USING (
    EXISTS (
